...
const addComment = async (postId: number, content: string, userId: string): Promise<Comment> => {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            const newComment = await addComment(Number(postId), content, userId);
            return res.status(201).json(newComment);
        } catch (error) {
            return res.status(500).json({ message: '댓글을 추가하는 데 실패했습니다.' });
        }
    }
