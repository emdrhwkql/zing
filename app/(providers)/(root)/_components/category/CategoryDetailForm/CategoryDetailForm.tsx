...
async function CategoryDetailForm({ categoryId }: { categoryId: number }) {
	...
	const followCategories =
		await api.followCategories.getFollowCategoriesByCategoryId(categoryId);

	// console.log(followCategories.count);

	const loungesByCategory = await api.lounges.getLoungesByCategoryId(
		categoryId
	);
